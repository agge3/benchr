    ...
    async def message_handler(message):
        ...
        try:
            ...
            if job_id in ws_clients:
                print(
                    f"[Quart] Found {len(ws_clients[job_id])} subscribers for job {job_id}",
                    flush=True,
                )
                ...
                if job_result is None:
                    print(
                        f"[Quart] WARNING: No job result found for job_id={job_id}",
                        flush=True,
                    )
                ...
                print(
                    f"[Quart] Broadcast complete, removed subscribers for job {job_id}",
                    flush=True,
                )
            else:
...
async def ws():
    ...
    try:
            ...
            while True:
                try:
                    ...
                    if data.get("type") == "subscribe":
                        ...
                        job_id = str(data.get("job_id"))
                        print(
                            f"[WS] Unsubscribe request for job_id={job_id}", flush=True
                        )
                        if job_id in subscribed_jobs:
                            ...
                            print(
                                f"[WS] Client unsubscribed from job {job_id}",
                                flush=True,
                            )

                    else:
                        print(
                            f"[WS] Unknown message type: {data.get('type')}", flush=True
                        )
        ...
        for task in done:
            if task.exception():
                print(
                    f"[WS] Task failed with exception: {task.exception()}", flush=True
                )
...
async def get_current_job():
    ...
    try:
        loop = asyncio.get_event_loop()
        job = await loop.run_in_executor(
            None, lambda: Job.select().order_by(Job.created_at.desc()).first()
        )
